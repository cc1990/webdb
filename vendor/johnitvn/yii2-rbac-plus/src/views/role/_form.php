...
foreach($servers as $server){
    $serverIds[$server['ip']] = $server['server_id'];
}
$sql_operations = array('DML','DDL','DQL');
$sharding_operations = array('DML','DDL','DQL');
...
//var_dump($model->serversPrivilege);exit;
?>
...
<div class="auth-item-form">
    ...
    <div class="form-group field-role-servers">
        <label class="control-label" for="role-servers">分库分表SQL操作</label>
        <div id="role-sqlshardingoperations">
            <?php $sharding_op=$model->sharding_operations; ?>
                <?php foreach ($sharding_operations as $operation): ?>
                <label><input <?= (@strpos("$sharding_op",$operation) !== false) ? "checked":"" ?> type="checkbox" name="Role[sharding_operations][]" value="<?= $operation ?>">&nbsp;<?= $operation ?></label>&nbsp;
                <?php endforeach; ?>
        </div>
        <div class="help-block"></div>
    </div>
    ...
    <div class="form-group field-role-servers">
        <label class="control-label" for="role-servers">通用库SQL操作</label>
        ...
        <div id="servers_privilege" class="privilege">
            <?php foreach($model->serversPrivilege as $serverIp=>$databases) :
                if($databases == 'all' || empty( $serverIds[$serverIp] ) )  continue;
            ?>
                <table id="table-<?=$serverIds[$serverIp]?>" server_ip = "<?=$serverIp?>" class="sui-table table-bordered table-zebra table-content-center">
                    <thead>
                        <tr><td style="width:1px" colspan="3" class="title"> <?=$serverIp?>-点击左侧服务列表条目，展开数据库:</td></tr>
                        <tr><td width="120px"><b>数据库名称</b></br><input type="text" name="search_database" placeholder="数据库筛选"></td><td><b>表</b></br><input type="text" name="search_table" placeholder="表筛选"></td></tr>
                    </thead>
                    <tbody>
                        <?php foreach($databases as $database=>$tables) : ?>
                        <tr>
                            <td><input type="checkbox" value="<?=$database?>" name="database[]" <?php if(isset($tables['_checked']) && $tables['_checked']) echo 'checked="checked"'; unset($tables['_checked'])?>><?=$database?></td>
                            <td>
                                <?php foreach($tables as $table=>$checked) : ?>
                                    <span class="privilege-table"><input type="checkbox" value="<?=$table?>" name="tables[]" <?php if($checked) echo 'checked="checked"'; ?>><?=$table?></span>
                                <?php endforeach; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
            ...
            <table class="sui-table table-bordered table-zebra">
                ...
                <tbody>
                    <?php foreach ($servers as $server): ?>
                        <tr>
                            <td>
                                <input <?= (isset($model->serversPrivilege[$server->ip]) && $model->serversPrivilege[$server->ip] == 'all') ? "checked":"" ?> type="checkbox" name="Role[servers][]" value="<?= $server->server_id ?>" server_ip = "<?=$server->ip?>">
                            </td>
                            <td ><?= $server->name ?></td>
...
<input type="hidden" id="hide_role_name" value="<?=$model->name?>">
<?php $this->registerJsFile("/public/js/common.function.js"); ?>
<?php $this->registerJsFile("/public/js/local/rbac_role_create.js"); ?>
