...
abstract class AuthItem extends Model {
    ...
    public function save() {
        ...
        if ($this->getType() == Item::TYPE_ROLE) {
            ...
            $servers->sharding_operations = (!empty($this->sharding_operations) && is_array($this->sharding_operations))?implode(",",$this->sharding_operations):'';
...
//            var_dump($result);exit;
    ...
    public function delete() {
        ...
        if ($this->getType() == Item::TYPE_ROLE) {
            ...
            $item_servers_dbs = new AuthItemServersDbs();
            $item_servers_dbs->deleteAll(['item_server_name'=>$this->name]);
    ...
    public function createRole($post){
        $type = Item::TYPE_ROLE;
        $now = time();
        $rulename = $post['rulename'] == '' ? NULL : "'{$post['rulename']}'";
        Yii::$app->getDb()->createCommand("insert into auth_item (name,type,description,data,created_at,updated_at)values('{$post['name']}',{$type},'{$post['description']}','',{$now},{$now})")->execute();
    }
