 ...
 * 一键部署数据库
 */
class DeploymentController extends SiteController
{
    public function actionIndex()
    {
        ...
        if(!empty($default_pro_id)){
            foreach($project_list as $key=>$val){
                if($default_pro_id != $val['id'])
                    unset($project_list[$key]);
            }
        }
        //传给视图
        $data['server_list'] = $server_list;
        $data['server_list_'] = $server_list_;
        $data['project_list'] = $project_list;
        $data['project_list_json'] = json_encode($project_list);
...
//                Output::error('数据库操作失败');
    ...
    public function actionGetSqlList( $server_id, $project_id )
    {
        ...
        $result = ExecuteLogs::find()->select(['log_id','script','notes', 'database', 'pro_id', 'user_id', 'created_date'])->where(['server_id' => $server_id, 'pro_id' => $project_id_array, 'sqloperation' => 'ddl'])->asArray()->all();
