...
class RedisServerWeb
{
    ...
    */
    ...
    /**
     * Description 删除hash key值
    */
    public function del($ip,$dabase,$table){
        if(!empty($table)){
            return $this->_redis->hdel($ip . "-" . $dabase,$table);
        }elseif(!empty($dabase)){
            return $this->_redis->del($ip . "-" . $dabase);
        }elseif(!empty($ip)){
            return $this->_redis->del($ip);
        }else{
            return false;
        }
    }
